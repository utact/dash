...
### 💡 Core Value
* **Zero-Manual Automation**: BOJ든 SWEA든, 문제 풀이 제출 시 자동 커밋 ＆ 코드 분석 ＆ 대시보드 갱신까지 원스톱으로 처리합니다.
...
2. **비동기 이벤트 처리**: GitHub 웹훅 수신 시, `GitHubPushEventWorker`로 분석 및 갱신 작업을 비동기 처리하여 사용자 경험을 저해하지 않습니다.
...
* **Time/Space Complexity**: Big-O 표기법 기반의 시간/공간 복잡도를 자동 계산하고, 그 이유를 명확히 설명합니다.

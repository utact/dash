...
class OAuthTokenServiceTest {
    ...
    @Mock
    private UserOAuthTokenMapper tokenMapper;
    ...
    void saveAccessToken_insertWhenAbsent() {
        when(tokenMapper.selectByUserId(USER_ID)).thenReturn(null);
    ...
    void saveAccessToken_updateWhenPresent() {
        ...
        when(tokenMapper.selectByUserId(USER_ID)).thenReturn(existing);
        ...
        ArgumentCaptor<UserOAuthToken> captor = ArgumentCaptor.forClass(UserOAuthToken.class);
        verify(tokenMapper).update(captor.capture());

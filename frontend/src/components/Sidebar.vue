          ...
          <transition name="slide-up">
            <div v-if="notificationsOpen" class="absolute left-full bottom-0 ml-4 w-80 bg-white border border-slate-100 rounded-2xl shadow-xl z-50 overflow-hidden">
               <div class="px-4 py-3 border-b border-slate-100 bg-slate-50 flex justify-between items-center">
                <h3 class="font-bold text-slate-800 text-sm">가입 신청</h3>
                <button @click="notificationsOpen = false" class="text-slate-400 hover:text-slate-600"><X :size="16"/></button>
              </div>
              <div v-if="pendingApplications.length === 0" class="p-6 text-center text-slate-400 text-xs">
                대기 중인 신청이 없습니다
              </div>
                ...
                <div v-for="app in pendingApplications" :key="app.id" class="p-4 border-b border-slate-50 last:border-b-0">
                  <div class="flex items-center gap-3 mb-2">
                    ...
                    <div class="flex-1 min-w-0">
                      <p class="font-bold text-slate-800 truncate text-sm">{{ app.applicant?.username }}</p>
                      <p class="text-xs text-slate-400">{{ formatAppDate(app.createdAt) }}</p>
                    </div>
                  </div>
                  <p v-if="app.message" class="text-xs text-slate-600 bg-slate-50 rounded-lg p-2 mb-3">{{ app.message }}</p>
                  <div class="flex gap-2">
                    <button @click="handleApprove(app.id)" class="flex-1 py-1.5 bg-brand-500 text-white text-xs font-bold rounded-lg hover:bg-brand-600 transition-colors">
                      승인
                    </button>
                    <button @click="handleReject(app.id)" class="flex-1 py-1.5 bg-slate-100 text-slate-600 text-xs font-bold rounded-lg hover:bg-slate-200 transition-colors">
                      거절
                    </button>
                  </div>
                </div>
...
const startPolling = () => {
  ...
  if (isStudyLeader.value) {
    fetchApplications();
    pollingInterval = setInterval(fetchApplications, 10000);
  }
};
const stopPolling = () => {
  if (pollingInterval) {
